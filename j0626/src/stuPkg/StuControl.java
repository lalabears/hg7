...
public class StuControl {
	...
	void scoreDelete() { 
		while(true) {
			...
			System.out.println("삭제를 원하는 학생의 이름을 입력해주세요 (종료0)");
			String name = scan.next();
			...
			for(int i = 0 ; i < list.size(); i++) {
				 if(list.get(i).getName().contains(name)) {
					System.out.println(list.get(i));
					stuno.add(i); // 몇번째 인덱스에 동명이인이 존재하는지 저장하는 리스트 
					// 동명이인의 인덱스를 저장함
				}
			}
			if(flag == 0) System.out.println("학생이 존재하지 않습니다. ");
			
			System.out.println("몇번 학생의 정보를 삭제하시겠습니까 ? ");
			String sno = scan.next(); // 학번을 입력받음(동명이인일경우 학번이 고유함)
			for(int i = 0 ; i< stuno.size();i++) {
				if(list.get(stuno.get(i)).getStuNo()==Integer.parseInt(sno.trim())) {
					System.out.println("정말로 삭제하시겠습니까? 1:네 2: 아니오");
					int choice = scan.nextInt();
					if(choice == 1) {
						list.remove(list.get(stuno.get(i)));
						System.out.println(list.get(stuno.get(i)).getName()+"학생의 정보가 삭제되었습니다. ");
					}else if(choice == 2){
	...
	void searchPrint() { 
		while(true) {
			...
			System.out.println("검색을 원하는 학생의 이름을 입력해주세요(종료0)");
			String name = scan.next();
			if(name.equals("0")) return; //메서드 종료 
			printTitle();
			for(int i = 0 ; i < list.size(); i++) {
				if(list.get(i).getName().contains(name)) {
					System.out.println(list.get(i));
